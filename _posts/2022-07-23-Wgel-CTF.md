...
## PORT 80
Using ffuf I do a recursive directory bruteforce scan which yields fruits...there is a private key hosted on the server that I think will help to login via ssh
~~~
┌──(wolf㉿piratesec)-[~/BOXES/thm/WgelCTF]
...
Also checking the default page on port 80 I found a name which could prove useful later
...
Checking what I can do with sudo, I find jessie can run "/usr/bin/wget" without asking for the passwd.
~~~
...
## Download the files
Start a netccat listener on my machine and output the content to files which I'll edit.
~~~
